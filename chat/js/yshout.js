...
//				'<input id="ys-input-nickname" value="' + this.prefs.defaultNickname + '" type="text" accesskey="N" maxlength="' + this.prefs.nicknameLength + '" class="ys-before-focus" />' +
	...
	send: function() {
		if (sendDelay == true) {
			...
			//var timeoutsend = setTimeout(timeoutCheck(), 5000); //##########
	...
	post: function(post) {
		...
		var html = 
			'<div id="' + id + '" class="ys-post'+ (post.banned ? ' ys-banned-post' : '') + '">' +
				(this.prefs.timestamp> 0 ? '<span class="ys-post-timestamp" style="color:#' + post.uid.substr(0,6) + ';">' + time(post.timestamp) + '</span> ' : '') +
...
//				'<span class="ys-post-actions"><a title="Show post information" class="ys-info-link" href="#">Info</a>'  + (post.adminInfo ? ' | <a title="Delete post" class="ys-delete-link" href="#">Удалить</a> | ' + (post.banned ? '<a title="Unban user" class="ys-ban-link" href="#">Разбан</a>' : '<a title="Ban user" class="ys-ban-link" href="#">Бан</a>') : '') + '</span>' +
				(post.adminInfo ? ' <span class="ys-post-actions"><a title="Delete post" class="ys-delete-link" href="#">Delete</a> | ' + (post.banned ? '<a title="Unban user" class="ys-ban-link" href="#">Unban</a>' : '<a title="Ban user" class="ys-ban-link" href="#">Ban</a>') : '') + '</span>' +
			'</div>';
	...
	smileys: function(s) {
		...
		s = smile(s, ';)',  'wink.gif');
		s = smile(s, ':nigra:',  'nigra.gif');
		s = smile(s, ':pet:',  'pet.gif');
		s = smile(s, ':nomad:',  'nomad.gif');
		s = smile(s, ':troll:',  'troll.gif');
		s = smile(s, ':f:',  'f.gif');
		s = smile(s, ':p:',  'petr.gif');
		s = smile(s, ':b:',  'b.gif');
		s = smile(s, ':il:',  'il.gif');
		s = smile(s, ':siga:',  'siga.gif');
		s = smile(s, ':h:',  'huy.gif');
		s = smile(s, ':t:',  'tian.gif');
		s = smile(s, ':vl:',  'train.gif');
		s = smile(s, ':rage:',  'rage.gif');
		s = smile(s, ':slow:',  'slow.gif');
		s = smile(s, ':pist:',  'pist.gif');
		s = smile(s, ':bm:',  'bm.gif');
		s = smile(s, ':cancer:',  'cancer.gif');
		s = smile(s, ':fp:',  'fp.gif');
		return s;
	},

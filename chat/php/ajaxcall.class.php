	...
	class AjaxCall {
		...
		function process() {
			switch($this->reqType) {
				...
				case 'post':
                                        if ($_SESSION['userenter'] != 1){
                                            break;
                                        }
                                        $nickname = $_SESSION['title'];
                                        $chatcolor = $_SESSION['chatcolor'];
                                        if (isset($_SESSION['nickhash'])){
                                        ...
                                        $pattern = '/(.*:\S{1,3}:.*){6,20}/';
                                        $replacement = '$1';
					$message = preg_replace($pattern, $replacement, $_POST['message']);
                                        $ChatTimerPost = time();
					cookie('yNickname', $nickname);
					$ys = ys($_SESSION['yLog']);
					if ($ys->banned(ip())) { $this->sendBanned(); break; }
					if ( isset($_SESSION['ChatTimerStart']) == false  ){
						$_SESSION['ChatTimerStart'] = time();
						if ($post = $ys->post($nickname, $message, $level, $chatcolor, $nickhash))	// To use $post somewheres later
								$this->sendUpdates();
					} else if ( ($ChatTimerPost - $_SESSION['ChatTimerStart']) > 4 )
					{
						if ($post = $ys->post($nickname, $message, $level, $chatcolor, $nickhash))	// To use $post somewheres later
								$this->sendUpdates();
								$_SESSION['ChatTimerStart'] = time();
